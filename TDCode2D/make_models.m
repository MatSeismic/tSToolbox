function [ model_stats, pdf_set_plot ] = make_models( models, xMat, yMat, sig_flag, pdf_x, unc_level, interp_style)
        %fine_models(k).tS    = griddata(models(k).xCell, models(k).yCell, models(k).tSCell, xMat, yMat, interp_style);

    warning off MATLAB:scatteredInterpolant:DupPtsAvValuesWarnId
    
    for k = 1:length(models)
        
        disp([num2str(length(models)) ' ' num2str(k) ]); 
        ...
        cellDistanceMat(:, :, k) = reshape(cellDistancevec, size(xMat));
    ...
    model_stats.nodeDistance = mean(cellDistanceMat, 3);
    ...
    for k = 1:numel(xMat)
        ...
        disp([num2str(numel(xMat)) ' ' num2str(k) ]);
        ...
        %pdf_set = histcounts(squeeze(modelset(i,j,:)), pdf_x, 'Normalization', 'pdf');
